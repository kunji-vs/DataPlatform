...
namespace DataPlatform.Drive
{
    ...
    public class OpcUaClient : INotifyPropertyChanged
    {
        ...
        /// <summary>
        /// 实例化一个新的OPC UA类,通过用户名和密码进行认证
        /// </summary>
        public OpcUaClient(string username, string pwd) : this()
        {
            if (!string.IsNullOrEmpty(username) && !string.IsNullOrEmpty(pwd))
            {
                UserIdentity = new UserIdentity(username, pwd);
            }
        }


        public OpcUaClient(X509Certificate2 certificate) : this()
        {
            UserIdentity = new UserIdentity(new X509Certificate2());
        }
