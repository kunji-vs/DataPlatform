...
{
    public class DriveModBusRtuOverTCP : IDrive
    {
        ModbusRtuOverTcp Device;
        ...
        public DriveModBusRtuOverTCP(int deviceId, string deviceName, string ip, int port, byte station, bool isPLCAddress = false, string DataFormat = "CDBA")
        {
        ...
        public bool Connect()
        {
            Device = new ModbusRtuOverTcp(_IP, _Port, _Station);
            Device.AddressStartWithZero = _AddressStartWithZero;
            Device.DataFormat = DriveModBusTCP.GetDataFormat(_DataFormat);
            readWriteNet = Device;
            Device.ConnectClose();
